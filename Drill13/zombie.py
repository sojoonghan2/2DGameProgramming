...
class Zombie:
    ...
    def is_boy_nearby_but_lose(self, r):
        if self.ball_count < play_mode.boy.ball_count:
            if self.distance_less_than(play_mode.boy.x, play_mode.boy.y, self.x, self.y, r):
                return BehaviorTree.SUCCESS
            else:
    ...
    def escape_to_boy(self, r=0.5):
        self.state = 'Walk'
        self.move_slightly_to(1000, 1000)
        if self.distance_less_than(play_mode.boy.x, play_mode.boy.y, self.x, self.y, r):
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.RUNNING
    ...
    def build_behavior_tree(self):
        ...
        c1 = Condition('소년이 근처에 있고, 공의 개수가 더 많은가?', self.is_boy_nearby, 7)
        a4 = Action('소년한테 접근', self.move_to_boy)
        SEQ_chase_boy = Sequence('소년을 추적', c1, a4)

        c2 = Condition('소년이 근처에 있고, 공의 개수가 더 적은가?', self.is_boy_nearby_but_lose, 7)
        a5 = Action('소년한테서 도망', self.escape_to_boy)
        SEQ_escape_boy = Sequence('소년에게서 도망', c2, a5)

        root = SEL_chase_or_flee = Selector('추적 또는 도망 또는 배회', SEQ_chase_boy, SEQ_escape_boy, SEQ_wander)
